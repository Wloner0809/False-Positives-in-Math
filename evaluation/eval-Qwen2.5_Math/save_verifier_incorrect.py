...
def save_verifier_incorrect(
    ...
    for sample_size_idx in range(len(sample_size)):
        BoN_Reward_incorrect_problem_id = [
            idx for idx, _ in best_of_n_prm_reward_sample_size[sample_size_idx]
        ]
        df_BoN_Reward_incorrect = pd.DataFrame(
            {
                "id": BoN_Reward_incorrect_problem_id,
                "problem": [problem_set[i] for i in BoN_Reward_incorrect_problem_id],
                "ground_truth": [
                    ground_truth[i] for i in BoN_Reward_incorrect_problem_id
                ],
                "solution": [
        ...
        SC_Reward_incorrect_problem_id = [
            idx for idx, _ in self_consistency_prm_reward_sample_size[sample_size_idx]
        ]
        df_SC_Reward_incorrect = pd.DataFrame(
            {
                "id": SC_Reward_incorrect_problem_id,
                "problem": [problem_set[i] for i in SC_Reward_incorrect_problem_id],
                "ground_truth": [
                    ground_truth[i] for i in SC_Reward_incorrect_problem_id
                ],
                "solution": [
                    unextracted_solutions_sample_size[sample_size_idx][problem_id][idx]
        ...
        if result_path is not None:
            if "Skywork-o1-Open-PRM-Qwen-2.5-7B" in prm_reward_path:
                df_BoN_Reward_incorrect.to_json(
                    f"{result_path}/BoN_Reward_incorrect_{sample_size[sample_size_idx]}samples-Skywork-o1-Open-PRM-Qwen-2.5-7B.jsonl",
                    orient="records",
                    lines=True,
                )
                df_SC_Reward_incorrect.to_json(
                    f"{result_path}/SC_Reward_incorrect_{sample_size[sample_size_idx]}samples-Skywork-o1-Open-PRM-Qwen-2.5-7B.jsonl",
                    orient="records",
                    lines=True,
                )
            elif "Qwen2.5-MATH-RM-72B" in prm_reward_path:
                df_BoN_Reward_incorrect.to_json(
                    f"{result_path}/BoN_Reward_incorrect_{sample_size[sample_size_idx]}samples-Qwen2.5-MATH-RM-72B.jsonl",
                    orient="records",
                    lines=True,
                )
                df_SC_Reward_incorrect.to_json(
                    f"{result_path}/SC_Reward_incorrect_{sample_size[sample_size_idx]}samples-Qwen2.5-MATH-RM-72B.jsonl",
                    orient="records",
                    lines=True,
                )
            elif "Skywork-PRM-vllm0.6.4.post1" in prm_reward_path:
