...
class TSNEAnalyzer:
    ...
    def __init__(self, random_state: int = 42):
        ...
        self.label_colors = {0: "#2E8B57", 1: "#FF6B6B", 2: "#4169E1", 3: "#FFA500"}
    ...
    def perform_tsne(
        ...
        max_perplexity = (len(features) - 1) // 4
        ...
        for method in preprocessing_methods:
            try:
                ...
                tsne_result = self.perform_tsne(
                    processed_features,
                    perplexity=min(30, (len(features) - 1) // 4),
                    n_iter=2000,
                )
        ...
        for i, perp in enumerate(perplexities):
            ...
            for label in np.unique(labels):
                ...
                axes[0, i].scatter(
                    tsne_results[mask, 0],
                    tsne_results[mask, 1],
                    c=self.label_colors.get(label, f"C{label}"),
                    label=self.label_names.get(label, f"Label_{label}"),
                    alpha=0.7,
                    s=30,
                )
    ...
    def visualize_trajectory_analysis(
        ...
        for label in np.unique(labels):
            ...
            if layer_tsne_results:
                # Dynamic axis mapping based on number of unique labels
                unique_labels = np.unique(labels)
                n_labels = len(unique_labels)

                # Create a mapping from label to subplot position
                if n_labels <= 4:
                    # Use 2x2 grid for up to 4 labels
                    label_to_idx = {unique_labels[i]: i for i in range(n_labels)}
                    ax_positions = [(0, 1), (1, 0), (1, 1), (0, 0)]
                    ax_idx = ax_positions[label_to_idx[label]]
                    ax = axes[ax_idx]
                else:
                    # For more than 4 labels, use modulo to wrap around
                    label_to_idx = {unique_labels[i]: i for i in range(n_labels)}
                    ax_idx = label_to_idx[label] % 4
                    ax_positions = [(0, 1), (1, 0), (1, 1), (0, 0)]
                    ax = axes[ax_positions[ax_idx]]
                ...
                ax.set_title(
                    f"{self.label_names.get(label, f'Label_{label}')} - Layer Progression"
                )
                ax.legend()
    ...
    def interactive_visualization(
        ...
        df_tsne["label"] = [
            self.label_names.get(label, f"Label_{label}") for label in labels
        ]
        ...
        # Create enhanced color mapping for all labels in the data
        unique_labels = np.unique(labels)
        color_map = {}
        default_colors = [
            "#2E8B57",
        ...
        for i, label in enumerate(unique_labels):
            ...
            if label in self.label_colors:
                color_map[label_name] = self.label_colors[label]
            else:
                color_map[label_name] = default_colors[i % len(default_colors)]
        ...
        # Enhanced marker styling with dynamic sizing
        unique_label_names = [
            self.label_names.get(label, f"Label_{label}") for label in unique_labels
        ]
        label_sizes = {}
        for label_name in unique_label_names:
            # Slightly larger size for better visibility
            label_sizes[label_name] = 10
        ...
        fig.update_layout(
            ...
            legend=dict(
                ...
                itemsizing="constant",
                title=dict(
                    text="<b>Error Type Categories</b>",
            ...
            fig.write_html(
                ...
                config={
                    ...
                    "toImageButtonOptions": {
                        "format": "png",
                        "filename": "tsne_false_positive_types",
                        "height": 700,
                        "width": 1000,
                        "scale": 2,
                    },
        ...
        # Try to show the figure with enhanced error handling
        try:
            fig.show(config={"displayModeBar": True, "displaylogo": False})
        except ValueError as e:
            if "nbformat" in str(e):
                print(f"Warning: Cannot display interactive plot - {e}")
                print(
                    "The interactive plot has been saved to HTML file if save_path was provided."
                )
                print("To fix this, install nbformat: pip install nbformat>=4.2.0")
            else:
                raise e
        except Exception as e:
            print(f"Warning: Cannot display interactive plot - {e}")
...
def load_sample_data(data_path: str = "sample_math_data.jsonl") -> tuple:
    ...
    labels = df["false_positive_type"].tolist()
