...
def generate_k_steps(
    ...
    lm_calling_config: LMCallingConfig,
    beam_width: int,
) -> list[Beam]:
    gen_results = []
    ...
    gen_sampling_params = copy.deepcopy(lm_calling_config)
    ...
    for i in range(lookahead_steps + 1):
        ...
        llm_outputs = []
        for current_generation, gen_prompt in zip(current_gen, gen_prompts):
            llm_outputs.append(
                (
                    current_generation,
                    policy_model_caller(gen_prompt, gen_sampling_params),
                )
            )
        for gen_result, output in llm_outputs:
