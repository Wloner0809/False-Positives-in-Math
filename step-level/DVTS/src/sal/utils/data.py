...
def get_dataset(config: Config) -> Dataset:
    if os.path.isfile(config.dataset_name):
        dataset = load_dataset(
            "json", data_files=config.dataset_name, split=config.dataset_split
        )
    else:
...
def save_dataset(dataset, config):
    if config.push_to_hub:
        ...
        dataset.to_json(
            f"{config.output_dir}/{model_name}_{dataset_name}_{config.approach}_{config.n}completions.jsonl",
            lines=True,
        )
        logger.info(
            f"Saved completions to {config.output_dir}/{model_name}_{dataset_name}_{config.approach}_{config.n}completions.jsonl"
        )
