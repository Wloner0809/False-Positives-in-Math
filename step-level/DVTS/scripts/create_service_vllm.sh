...
do
    echo "Starting policy worker $i"
    WORKER_PORT=$((WORKER_BASE_PORT+i))
    tmux new-window -n policy_worker_$i
    tmux send-keys "cd src/sal" Enter
    tmux send-keys "export LOGDIR=${LOGDIR}" Enter
    tmux send-keys "CUDA_VISIBLE_DEVICES=$((i+CUDA_DEVICE_BASE)) $PYTHON_EXECUTABLE -m llm_service.policy_model_worker --model-path $MODEL_PATH --controller-address http://$HOST_ADDR:$CONTROLER_PORT --host $HOST_ADDR --port $WORKER_PORT --worker-address http://$HOST_ADDR:$WORKER_PORT --gpu_memory_utilization 0.7" Enter
done